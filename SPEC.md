...
- V80: movement DIRECTION is STICKY (press left/right → move until `stop`/opposite), JUMP stays momentary (one pulse/press). a terminal auto-repeats only ONE key, so a decaying direction (V73) can't coexist with a jump — sticky direction makes left/right COMBINABLE with jump/fire/hook via separate presses (B13). explicit `stop` + opposite end movement; jump ⊥ latch (B10 retained). (C34; supersedes V73 direction-decay)
