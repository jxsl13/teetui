...
T126|.|map-download progress (§C39/gh#1, C16): FILE jxsl13/twclient issues — (i) `Connect` silently succeeds without map on download failure (should error/retry or expose a "no map" state), (ii) expose a map-download PROGRESS callback/event (bytes/total). link the issue numbers back into §B18. when twclient emits progress → render a progress bar during the join/download phase (replaces the indeterminate spinner). (now: process task; render blocked on twclient)|C39,V86,I.twclient
id|date|cause|fix
...
B18|2026-06-16|(gh #1) connected but NO map render → eternal "connecting…": twclient `Connect` includes the map download and SUCCEEDS even when it fails (`client/client.go:425` "continuing without map"), so teetui is `connected` with `st.Map==nil` → chat works but the game window shows the connecting spinner forever; reconnect re-downloads OK. fix (teetui): connected-no-map → actionable "press R to re-download" notice; FILE twclient issues for the silent map-download failure + a progress callback (C16)|V86
