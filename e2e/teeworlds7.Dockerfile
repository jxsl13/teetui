# Vanilla teeworlds 0.7.5 dedicated server for the teetui e2e harness, BUILT FROM
# SOURCE with the Debug build enabled (SPEC §C14). Backs teetui's vanilla-0.7
# table case (teeworlds7:8303) — a genuine pure-0.7 server (no sixup).
#
...
# tee. teeworlds' CMake defines CONF_DEBUG for the Debug config, so a Debug build
# registers `dbg_dummies` → bot characters → multi-character snapshots, on top of
# the CTF flags/pickups from the ctf1 map.
