# Vanilla DDNet sixup server for the teetui e2e harness, BUILT FROM SOURCE with
# the debug build enabled (SPEC §C14). Serves BOTH teeworlds 0.6 and 0.7 clients
# on one UDP port (sixup), so the same container backs the ddnet-0.6 and
# ddnet-0.7-sixup table cases that teetui's e2e suite connects against.
#
...
# only the connecting client's own tee. A `-DDEV=ON` Debug build defines
# CONF_DEBUG → registers `dbg_dummies`, so the server spawns bot characters and
# every snapshot is genuinely multi-character (richer roster for teetui to log).

# ---- builder: compile DDNet-Server (server only, debug) --------------------
# DDNet (19.x) compiles Rust components, so the builder needs a current Rust +
# Cargo (CMakeLists "You must install Rust and Cargo to compile DDNet"). The
# official rust:bookworm image bundles a recent toolchain on Debian, onto which
# we add the C++ build deps.
...
# RelWithDebInfo silently omits dbg_dummies. DEV=ON enables the dev maps.
...
# (CONF_DEBUG build) so 4 bots spawn → multi-character snapshots. sv_sixup 1
# serves both 0.6 and 0.7 on the one UDP port (ddnet:8303), backing teetui's
# ddnet-0.6 and ddnet-0.7-sixup cases from a single container.
CMD ["./DDNet-Server", \
     "sv_name 'teetui-e2e ddnet source-debug sixup'", \
     "sv_port 8303", \
     "sv_register 0", \
     "sv_max_clients 64", \
